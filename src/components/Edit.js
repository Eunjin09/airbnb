...
import { updatePostDB } from "../redux/modules/listSlice";
...
function Write() {
  ...
  useEffect(() => {
    async function load() {
      await dispatch(ListActions.loadDetailDB(params.id));
      setIsloaded(true);
    }
    load();
  }, []);
            ...
            <ScreenRight>
              <InputBox>
                ...
                <Inputarea
                  ...
                  value={address || ""}
                  onChange={onChange}
                  placeholder={housedraft[0].address}
                ></Inputarea>
                <button
                  onClick={() => {
                    setOpened(true);
              ...
              <InputBox>
                ...
                <Inputarea
                  ...
                  value={houseName}
                  onChange={onChange}
                  placeholder={housedraft[0].houseName}
                />
              </InputBox>
              <InputBox>
                <Textarea>
                  5개의 사진을 올려주세요! (사진은 초기화 됩니다.)
                </Textarea>
              ...
              <InputBox>
                ...
                <Inputarea
                  ...
                  value={enteredNum || ""}
                  onChange={priceComma}
                  placeholder={housedraft[0].price}
                />
              </InputBox>
              <InputBox>
                ...
                <Inputarea
                  ...
                  value={houseInfo}
                  onChange={onChange}
                  placeholder={housedraft[0].houseInfo}
                />
              </InputBox>
              <InputBox>
                <Select name="personCnt" value={personCnt} onChange={onChange}>
                  <option>{housedraft[0].personCnt}</option>
                  <option>1</option>
                  <option>2</option>
                  <option>3</option>
                  <option>4</option>
                ...
                <Select name="wifi" value={wifi} onChange={onChange}>
                  <option>
                    {housedraft[0].wifi ? "와이파이 있음" : "와이파이 없음"}
                  </option>
                ...
                <Select name="parking" value={parking} onChange={onChange}>
                  <option>
                    {housedraft[0].parking ? "주차 가능" : "주차 불가능"}
                  </option>
                  <option value={1}>주차 가능</option>
                  <option value={0}>주차 불가능</option>
                </Select>
              </InputBox>
              <Submit onClick={updateSubmit}>수정 하기</Submit>
            </ScreenRight>
            <ScreenLeft>
              <h1>수정 하기!</h1>
            </ScreenLeft>
